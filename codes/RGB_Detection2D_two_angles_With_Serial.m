...
while(nFrame < 200)
    ...
    for object = 1:1:length(bboxBlue(:,1)) % Write the corresponding centroids for blue
        ...
        vidIn = step(htextinsCent, vidIn, [centXBlue centYBlue], [centXBlue-6 centYBlue-9]); 
         ...
         vidIn = step(line, vidIn, [centXBlue,centYBlue,centXRed,centYRed],single([0 1 1]));        %Arm Skeltone
         vidIn = step(line, vidIn, [centXGreen,centYGreen,centXBlue,centYBlue],single([0 1 1]));    %Arm Skeltone
         
         vidIn = step(line, vidIn, [centXGreen-20,centYGreen,centXGreen+20,centYGreen],single([1 1 1]));        %Axiz for angle measuring
         vidIn = step(line, vidIn, [centXGreen,centYGreen-20,centXGreen,centYGreen+20],single([1 1 1]));
     ...
     angle1= 180-angle1;
     angle2= 180-angle2;
     str = strcat('$',num2str(angle1,'%03i'),'*',num2str(angle2,'%03i'),'#')
